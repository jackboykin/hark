...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    // ALPN extension setup (RFC 7301)
    const alpn_name = options.alpn orelse "";
    const alpn_name_len: u16 = @intCast(alpn_name.len);
    // Extension header: type(2) + ext_data_len(2) + proto_list_len(2) + proto_entry_len(1) = 7 bytes
    const alpn_ext_header = int(u16, @intFromEnum(tls.ExtensionType.application_layer_protocol_negotiation)) ++
        int(u16, 2 + 1 + alpn_name_len) ++ // extension data length
        int(u16, 1 + alpn_name_len) ++ // protocol list length
        [1]u8{@intCast(alpn_name_len)}; // protocol entry length
    ...
    const cleartext_header_buf = .{@intFromEnum(tls.ContentType.handshake)} ++
        ...
        int(u16, @intCast(out_handshake.len - server_name_extension.len + server_name_extension_len + alpn_extension_len)) ++
        out_handshake;
...

    {
        ...
        if (options.alpn != null) {
            iovecs[iov_n] = &alpn_ext_header;
            iov_n += 1;
            iovecs[iov_n] = alpn_name;
            iov_n += 1;
        }
        try output.writeVecAll(iovecs[0..iov_n]);
        try output.flush();
    }
                    ...
                    .server_hello => {
                        ...
                        switch (cipher_suite_tag) {
                            ...
                            => |tag| {
                                ...
                                if (options.alpn != null) { // Client Hello part 3 (ALPN)
                                    p.transcript_hash.update(&alpn_ext_header);
                                    p.transcript_hash.update(alpn_name);
                                }
