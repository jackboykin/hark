...
print(f"{'':8}{'hit qps':>10}{'busy%':>6}{'ok%':>6}  {'miss qps':>9}{'busy%':>6}{'ok%':>6}  {'p50µs':>6}{'p99µs':>7}{'p99.9':>7}  runs")
for n, d in sorted(rows.items(), key=lambda x: -med(x[1]['hit_qps'])):
    print(f"{n:8}{med(d['hit_qps']):>10.0f}{med(d['hit_busy']):>6.1f}{med(d['hit_ok']):>6.1f}  {med(d['miss_qps']):>9.0f}{med(d['miss_busy']):>6.1f}{med(d['miss_ok']):>6.1f}  {med(d['p50']):>6.0f}{med(d['p99']):>7.0f}{med(d['p99.9']):>7.0f}  {len(d['hit_qps'])}  hit={[round(x/1000) for x in d['hit_qps']]}k")
